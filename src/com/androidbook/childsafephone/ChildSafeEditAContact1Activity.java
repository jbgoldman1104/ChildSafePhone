...
public class ChildSafeEditAContact1Activity extends ChildSafeActivity {
	
	SharedPreferences contactInfo;
	static final int BROWSE_DIALOG_ID = 0;
	int initIcon = CHILDSAFE_PREFERENCES_CONTACT1ICON;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		editContactEditText.setText(contactInfo.getString(CHILDSAFE_PREFERENCES_CONTACT1NAME, ""));
		editContactEditNumber.setText(contactInfo.getString(CHILDSAFE_PREFERENCES_CONTACT1PHONE, ""));
		editIcon.setImageResource(contactInfo.getInt("CHILDSAFE_PREFERENCES_CONTACT1ICON", R.drawable.icon00));
		
		//retrieves the original icon value
		initIcon= contactInfo.getInt("CHILDSAFE_PREFERENCES_CONTACT1ICON", R.drawable.icon00);
	...
	private void  buttonOKEditScreen(){
        ...
        cancelEditAContactButton.setOnClickListener(new View.OnClickListener() {
            public void onClick(View v) {
				...
				String strEditContactName = editContactEditText.getText().toString();
                String strEditContactNumber = editContactEditNumber.getText().toString();
                   	
                	//Commit the following changes when clicked ok
                	Editor editor = contactInfo.edit();
                    editor.putString(CHILDSAFE_PREFERENCES_CONTACT1NAME, strEditContactName);
                    editor.commit();
                    editor.putString(CHILDSAFE_PREFERENCES_CONTACT1PHONE, strEditContactNumber);
                    editor.commit();
                    startActivity(new Intent(ChildSafeEditAContact1Activity.this,ChildSafeParentSettingActivity.class));
            }
        });
	}
	...
	private void  buttonCancelEditScreen(){
        ...
        cancelEditAContactButton.setOnClickListener(new View.OnClickListener() {
            public void onClick(View v) {
            	...
            	Editor editor = contactInfo.edit();
                editor.putInt("CHILDSAFE_PREFERENCES_CONTACT1ICON", initIcon);
                editor.commit();
                
                startActivity(new Intent(ChildSafeEditAContact1Activity.this,ChildSafeEditContactActivity.class));
            }
        });
	}
	
	//the Browse Button Handling
			...
			for (int curIcon = 0; curIcon <= 9; curIcon++){
				...
				curImgView.setOnClickListener(new View.OnClickListener() {
					public void onClick(View v) {			
		                ...
		                //remove the dialog
		                ChildSafeEditAContact1Activity.this.removeDialog(BROWSE_DIALOG_ID);
					}
				});	
			}
			...
			builder.setNegativeButton(android.R.string.cancel,
					new DialogInterface.OnClickListener() {
						public void onClick(DialogInterface dialog,
								int whichButton) {			 	                
							ChildSafeEditAContact1Activity.this
